...
function changeDistrict() {
  const kelurahanArea =
    "select#" +
    (kiriofBillingAddressConfig.fieldKey || "kiriof_destination_area") +
  ...
  jQuery(kelurahanArea)
    ...
    .on("change.kiriofClassicDistrict", function () {
      ...
      const selectedDistrictLabel = kiriofGetClassicDistrictLabel(root, kiriofBillingAddressConfig);
      const ajaxurl =
        typeof kiriofAjax !== "undefined" && kiriofAjax.ajaxurl ?
          kiriofAjax.ajaxurl
        : kiriofBillingAddressConfig.ajaxUrl || "";
...
function getSearchAreaKelurahan() {
  const subDistrictSelectElem = jQuery(
    `[name="${kiriofBillingAddressConfig.fieldKey || "kiriof_destination_area"}"],[name=kiriof_shipping_destination_area]`,
  );
  const ajaxurl =
    typeof kiriofAjax !== "undefined" && kiriofAjax.ajaxurl ?
      kiriofAjax.ajaxurl
  ...
  subDistrictSelectElem.each(function () {
    ...
    select2.call($field, {
      ...
      placeholder:
        kiriofBillingAddressConfig.i18n.selectOption || "Select Option",
      allowClear: true,
      ajax: {
        url: ajaxurl,
        dataType: "json",
