<script lang="ts">
    ...
    function initialAccountState(): Pick<
        AccountBootstrap,
        ...
        try {
            ...
            message =
                error instanceof Error
                    ? error.message
                    : bootstrap.i18n.connectionFailed;
        } finally {
            busy = false;
        }
    }
                ...
                {#each bootstrap.couriers as courier (courier.code)}
                    <ActionTooltip label={courier.name}>
                        {#if courierImage(courier.code, courier.name)}
                            <img
                                src={courierImage(courier.code, courier.name)}
                                class="kiriof-courier-logo"
                                alt={courier.name}
                            />
                        {:else}
            ...
            <div class="kiriof-connection-form">
                    ...
                    <div class="kiriof-linked-profile">
                        <div>
                            <div class="kiriof-linked-profile__name">
                                ...
                                {#if profile.paymentMethod}<span
                                        >{profile.paymentMethod}</span
                                    >{/if}
                            </div>
                            <span>{profile.email}</span>
                        </div>
                        <span class="kiriof-linked-profile__badge"
                            ><IconLink />{bootstrap.i18n.linkedAccount}</span
                        >
                    </div>
                {:else if connected}
                    <p class="kiriof-settings-root__message is-error">
                        {profileError
                            ? bootstrap.i18n.accountUnavailable
                            : bootstrap.i18n.connectedUnavailable}
                    </p>
                {/if}

                <label class="kiriof-connect__label" for="kiriof-account-key"
                    >{bootstrap.i18n.setupKey}
                ...
                <div class="kiriof-setup-key-field">
                    ...
                    <button
                        ...
                        aria-label={revealSetupKey
                            ? "Hide setup key"
                            : "Show setup key"}
                    >
                        {#if revealSetupKey}<IconEyeOff />{:else}<IconEye
                            />{/if}
                    </button>
                </div>
                ...
                <div class="kiriof-connection-consent">
                    ...
                    <div>
                        ...
                        <p>
                            {bootstrap.i18n.agreementPrefix}
                            <a
                                href={bootstrap.termsUrl}
                                target="_blank"
                                rel="noopener noreferrer"
                                >{bootstrap.i18n.terms}</a
                            >
                            {bootstrap.i18n.agreementAnd}
                            <a
                                href={bootstrap.privacyUrl}
                                target="_blank"
                                rel="noopener noreferrer"
                                >{bootstrap.i18n.privacy}</a
                            >.
                        </p>
                    </div>
                </div>
                ...
                {#if message}<p
                        class="kiriof-section-message error"
                        role="alert"
                    >
                        {message}
                    </p>{/if}
                ...
                <Button
                    class="kiriof-update-connection"
                    disabled={busy || !setupKey.trim()}
                    onclick={updateConnection}
                >
                    <IconLink
                        class="kiriof-update-connection__icon"
                        aria-hidden="true"
                    />
                    <span
                        >{busy
                            ? bootstrap.i18n.connecting
                            : connected
                              ? bootstrap.i18n.updateConnection
                              : bootstrap.i18n.connect}</span
                    >
                </Button>
            </div>
