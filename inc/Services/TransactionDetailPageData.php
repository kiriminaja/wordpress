...
class TransactionDetailPageData {
	...
	public function prepare( object $transaction ): array {
		...
		$order_total    = $wc_order ? (float) $wc_order->get_total() : $subtotal + max( 0.0, $shipping - $discount ) + $insurance + $cod_fee;
		$shipping_discount = $wc_order ? max( 0.0, $shipping - (float) $wc_order->get_shipping_total() ) : $discount;
		$paid_shipping  = $wc_order ? max( 0.0, (float) $wc_order->get_shipping_total() ) : max( 0.0, $shipping - $shipping_discount );
			...
			'transaction' => array(
				...
				'shipment'      => array(
					...
					'costs'         => array(
						...
						'insurance'        => $insurance,
						'codFee'           => $cod_fee,
						'itemDiscount'     => $wc_order ? (float) $wc_order->get_discount_total() : 0.0,
						'total'            => max( 0, $paid_shipping + $insurance + $cod_fee ),
					),
					'codValue'      => $cod_value,
					'printUrl'      => $print_url,
	...
	private function items( $order ): array {
		...
		foreach ( $order->get_items( 'line_item' ) as $item ) {
			...
			$image_url = $product && method_exists( $product, 'get_image_id' )
