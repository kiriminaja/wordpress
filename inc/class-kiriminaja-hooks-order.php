			...
			<div id="kiriminaja-request-pickup" style="display:none;width:600px;">
				<div class="kiriminaja-request-pickup">
					...
					<div class="pickup-time-wrapper">
						...
						<button type="button" class="button button-primary" id="kiriminaja-send-request-pickup"><?php esc_html_e( 'Send Request', 'kiriminaja' ) ?></button>
					</div>
				</div>
			</div>
	...
	public function additional_shipping_info( $order_id ) {
		...
		if ( $order->get_shipping_methods() ) {
			...
			<tr>
				...
				<td width="1%"></td>
				<td class="total">
					<span class="amount"><?php echo esc_html( $dimension['length'] . ' x ' . $dimension['width'] . ' x ' . $dimension['height'] . ' cm' ); ?></span>
				</td>
			</tr>
			<?php
		}
