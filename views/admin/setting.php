<div class="kiriminaja-setting <?php echo empty($settings['token']) ? 'unset' : ''; ?>">
    ...
    <div class="kiriminaja-api-setting">
        <div class="api-field">
            <input name="kiriminaja_setting[token]" type="text" class="input-text regular-input"
                   value="<?php echo esc_attr($settings['token']); ?>" <?php echo !empty($settings['token']) ? 'readonly' : ''; ?>>
            <?php if (empty($settings['token'])) : ?>
                <button class="button" type="button"
                        id="kiriminaja-check-key"><?php esc_html_e('Check Key', 'kiriminaja'); ?></button>
            <?php else: ?>
                <button class="button" type="button"
                        id="kiriminaja-delete-key"><?php esc_html_e('Delete Key', 'kiriminaja'); ?></button>
            <?php endif; ?>
        </div>
        <?php if (empty($settings['token'])) : ?>
        ...
        <table class="form-table">
            ...
            <tr valign="top">
                ...
                <td class="forminp forminp-text">
                    <input readonly type="text" maxlength="4" class="input-text regular-input"
                           value="<?php echo esc_attr($settings['ref_prefix']); ?>">
                </td>
            </tr>
            <tr valign="top">
                <th scope="row" class="titledesc"><?php esc_html_e('Store Name', 'kiriminaja'); ?></th>
                <td class="forminp forminp-text">
                    <input name="kiriminaja_setting[store_name]" type="text" class="input-text regular-input"
                ...
                <td class="forminp forminp-single_select_page_with_search">
                    <select name="kiriminaja_setting[store_province]" class="select">
                        ...
                        <?php foreach ($provinces as $key => $value) : ?>
                            <option
                                value="<?php echo esc_attr($key) ?>" <?php echo intval($key) === intval($settings['store_province']) ? 'selected' : ''; ?>><?php echo esc_html($value); ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
            </tr>
            ...
            <tr valign="top">
                ...
                <td class="forminp forminp-textarea">
                    <textarea
                        name="kiriminaja_setting[store_address]"><?php echo esc_textarea($settings['store_address']); ?></textarea>
                </td>
            </tr>
            <tr valign="top">
                <th scope="row" class="titledesc"><?php esc_html_e('Store Zipcode', 'kiriminaja'); ?></th>
                <td class="forminp forminp-text">
                    <input name="kiriminaja_setting[store_zipcode]" type="text" class="input-text regular-input"
                           pattern="^([1-9])[0-9]{4}$" value="<?php echo esc_attr($settings['store_zipcode']); ?>">
                </td>
            </tr>
            <tr valign="top">
                <th scope="row" class="titledesc"><?php esc_html_e('Store Phone', 'kiriminaja'); ?></th>
                <td class="forminp forminp-text">
                    <input name="kiriminaja_setting[store_phone]" type="text" class="input-text regular-input"
                           pattern="(\+62 ((\d{3}([ -]\d{3,})([- ]\d{4,})?)|(\d+)))|(\(\d+\) \d+)|\d{3}( \d+)+|(\d+[ -]\d+)|\d+"
                           minlength="10" value="<?php echo esc_attr($settings['store_phone']); ?>">
                </td>
            </tr>
            <tr valign="top">
                    ...
                    <?php foreach ($couriers as $courier) : ?>
                        <fieldset>
                            <label for="couriers-<?php echo esc_attr($courier->code); ?>">
                                <input name="kiriminaja_setting[couriers][]"
                                       id="couriers-<?php echo esc_attr($courier->code); ?>" type="checkbox"
                                       value="<?php echo esc_attr($courier->code); ?>" <?php echo in_array($courier->code, $settings['couriers'], true) ? 'checked' : ''; ?>> <?php echo esc_html($courier->name); ?>
                            </label>
                        </fieldset>
                    <?php endforeach; ?>
                </td>
