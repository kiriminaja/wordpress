...
foreach ($transactions as $kiriof_index => $kiriof_transaction){

    $kiriof_transactionCost = 0;
    $kiriof_transactionCost += intval($kiriof_transaction->shipping_cost ?? 0)+intval($kiriof_transaction->insurance_cost ?? 0);
    if ($kiriof_transaction->cod_fee > 0){
        $kiriof_transactionCost += intval($kiriof_transaction->cod_fee ?? 0)+intval($kiriof_transaction->transaction_value ?? 0);
    }
    $kiriof_destinationData = (object) json_decode($kiriof_transaction->shipping_info);
    ...
    echo '<table  style="width: 100%; height: 95%; border-collapse: collapse; margin-top: .25rem" border="1">
            ...
            <tr>
                <td style="border-right: 0;padding: .5rem; position: relative">
                    <div style="position: relative">
                        <strong style="display: block;margin-top: 0.5rem">'.esc_html($kiriof_transaction?->order_id).'</strong>
                    </div>
                    <div style="display: inline-block; margin-top: 1.25rem">
                        <div style="display: inline-block;border: 1px solid #000;padding: .15rem">
                            <img src="' . esc_url('https://kiriminaja-static-file.imgix.net/home-v3/logistics/' . $kiriof_transaction->service . '.png') . '" height="20px" alt="">'; // phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage
                        '</div>
                        <div style="display: inline-block;padding: .15rem .5rem">
                            Tipe Layanan
                            <br>
                            <strong>'.esc_html($kiriof_transaction?->service_name).'</strong>
                        </div>
                    </div>
                </td>
                    ...
                    <div style="margin-top: .75rem">
                        <div style="width: 30%;display: inline-block">
                         ...
                         <div style="width: 30%;display: inline-block">
                            Quantity
                            <br>
                            <strong>'.(esc_html($kiriof_transaction->item_count) ?? '-').'</strong>
                        </div>
                    </div>
                </td>
            </tr>
            ...
            <tr>
                <td style="padding: .5rem; width: 50%; border-right: 0">
                    Penerima
                    <br>
                    <strong style="font-size: .75rem;">'.(esc_html($kiriof_destinationData->_shipping_first_name) ?? esc_html($kiriof_destinationData->_billing_first_name) ).' '.(esc_html($kiriof_destinationData->_shipping_last_name) ?? esc_html($kiriof_destinationData->_billing_last_name)).'</strong>
                    <br>
                    '.(esc_html($kiriof_destinationData->_shipping_address_1) ?? esc_html($kiriof_destinationData->_billing_address_1)).' '.(esc_html($kiriof_destinationData->_shipping_address_2) ?? esc_html($kiriof_destinationData->_billing_address_2)).' '.esc_html($kiriof_transaction->destination_sub_district).' '.(esc_html($kiriof_destinationData->_shipping_postcode) ?? esc_html($kiriof_destinationData->_billing_postcode)).'
                    <br>
                    '.esc_html($kiriof_destinationData->_billing_phone).'
                </td>
            ...
            <tr>
                <td colspan="2" style="padding: .5rem; padding-bottom: 1rem">
                    Catatan:
                    <br>
                    <strong>'.(esc_html($kiriof_transaction->checkout_note) ?? '-').'</strong>
                </td>
            </tr>
