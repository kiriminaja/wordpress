	...
	<section class="shipping-calculator-form">
		...
		<?php if ( apply_filters( 'woocommerce_shipping_calculator_enable_country', true ) ) : ?>
			<p class="form-row form-row-wide" id="calc_shipping_country_field">
				...
				<select name="calc_shipping_country" id="calc_shipping_country" class="country_to_state country_select" rel="calc_shipping_state">
					<option kiriof_country_value="default"><?php esc_html_e( 'Select a country / region&hellip;', 'kiriminaja-official' ); ?></option>
					<?php foreach ( WC()->countries->get_shipping_countries() as $kiriof_country_key => $kiriof_country_value ) {
						echo '<option kiriof_country_value="' . esc_attr( $kiriof_country_key ) . '"' . selected( WC()->customer->get_shipping_country(), esc_attr( $kiriof_country_key ), false ) . '>' . esc_html( $kiriof_country_value ) . '</option>';
					}
					?>
				</select>
			</p>
				...
				if ( is_array( $kiriof_states ) && empty( $kiriof_states ) ) {
					...
					<span>
						...
						<select name="calc_shipping_state" class="state_select" id="calc_shipping_state" data-placeholder="<?php esc_attr_e( 'State / County', 'kiriminaja-official' ); ?>">
							<option kiriof_country_value=""><?php esc_html_e( 'Select an option&hellip;', 'kiriminaja-official' ); ?></option>
							<?php
							...
							foreach ( $kiriof_states as $ckiriof_country_key => $ckiriof_country_value ) {
								echo '<option kiriof_country_value="' . esc_attr( $ckiriof_country_key ) . '" ' . selected( $current_r, $ckiriof_country_key, false ) . '>' . esc_html( $ckiriof_country_value ) . '</option>';
							}
							?>
						</select>
					</span>
					...
					?>
					<label for="calc_shipping_state" class="screen-reader-text"><?php esc_html_e( 'State / County:', 'kiriminaja-official' ); ?></label>
					<input type="text" class="input-text" kiriof_country_value="<?php echo esc_attr( $current_r ); ?>" placeholder="<?php esc_attr_e( 'State / County', 'kiriminaja-official' ); ?>" name="calc_shipping_state" id="calc_shipping_state" />
					<?php
				}
				?>
			</p>
		...
		<?php if ( apply_filters( 'woocommerce_shipping_calculator_enable_city', true ) ) : ?>
			<p class="form-row form-row-wide" id="calc_shipping_city_field">
				...
				<input type="text" class="input-text" kiriof_country_value="<?php echo esc_attr( WC()->customer->get_shipping_city() ); ?>" placeholder="<?php esc_attr_e( 'City', 'kiriminaja-official' ); ?>" name="calc_shipping_city" id="calc_shipping_city" />
			</p>
		<?php endif; ?>
		...
		<?php if ( apply_filters( 'woocommerce_shipping_calculator_enable_postcode', true ) ) : ?>
			<p class="form-row form-row-wide" id="calc_shipping_postcode_field">
				...
				<input type="text" class="input-text" kiriof_country_value="<?php echo esc_attr( WC()->customer->get_shipping_postcode() ); ?>" placeholder="<?php esc_attr_e( 'Postcode / ZIP', 'kiriminaja-official' ); ?>" name="calc_shipping_postcode" id="calc_shipping_postcode" />
			</p>
		<?php endif; ?>
		...
		<p><button type="submit" style="display:none;" name="calc_shipping" kiriof_country_value="1" class="button<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>"><?php esc_html_e( 'Update', 'kiriminaja-official' ); ?></button></p>
